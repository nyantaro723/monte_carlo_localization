# Monte Carlo Localization: 

1次元ロボットの自己位置推定をするシミュレーション

[![Test](https://github.com/nyantaro723/monte_carlo_localization/workflows/Test/badge.svg)](https://github.com/nyantaro723/monte_carlo_localization/actions/workflows/test.yml)
[![Python](https://img.shields.io/badge/python-3.8%20%7C%203.9%20%7C%203.10%20%7C%203.11%20%7C%203.12-blue)](https://www.python.org/)
[![License](https://img.shields.io/badge/License-MIT-green)](LICENSE)

## 概要

このリポジトリは、**モンテカルロ法（パーティクルフィルタ）** を用いた1次元ロボット自己位置推定のシミュレータです。ロボットがセンサ観測と移動制御のみから自身の位置を推定する過程を、パーティクルフィルタアルゴリズムで実装しています。

### 何ができるのか？

- **自己位置推定**: ロボットが未知の環境で自身の位置を推定
- **パーティクルフィルタ** による逐次的な位置更新
- **可視化**: パーティクル分布の時系列変化とアニメーションを自動生成
- **カスタマイズ可能**: パラメータ調整で異なるシナリオをシミュレーション可能

---

## アルゴリズム: パーティクルフィルタ 

パーティクルフィルタは、**非線形・非ガウス過程** の状態推定に適した確率的アルゴリズムです。

### 動作原理

```
[初期化]
  → 位置不確定 → N個のパーティクルを全体に均等分布

[反復処理]
  ① 予測 
     制御入力からパーティクルを移動 + ノイズ付加
  
  ② 更新   
     センサ観測から各パーティクルの尤度を計算
     尤度に応じてパーティクルの重みを更新
  
  ③ リサンプリング 
     有効パーティクル数が少ない場合、重みに基づいてパーティクルを再抽出
     → 重みが高い（信頼度が高い）領域にパーティクルが集中
```

### 数式（予測・更新・正規化）

- 遷移モデル（運動方程式の一般形）:
$$p(x_t \mid x_{t-1}, u_t)$$

- 予測（この実装では 1次元直線＋ガウス雑音）:
$$x_t^{(i)} = x_{t-1}^{(i)} + u_t + w_t, \quad w_t \sim \mathcal{N}(0, \sigma_p^2)$$

- 重みの未正規化更新
$$w̃_t^{(i)} = w_{t-1}^{(i)} \, p(z_t \mid x_t^{(i)})$$

- 正規化:
$$w_t^{(i)} = \frac{w̃_t^{(i)}}{\sum_j w̃_t^{(j)}}$$

- 有効パーティクル数（リサンプリング判定に利用）:
$$N_{\text{eff}} = \frac{1}{\sum_i (w_t^{(i)})^2}$$

ここで：

| 記号 | 意味 |
|-----|------|
| $x_t^{(i)}$ | 時刻 $t$ のパーティクル $i$ の状態（1次元位置） |
| $u_t$ | 時刻 $t$ の制御入力（移動距離） |
| $w_t$ | 運動モデルの雑音 $\mathcal{N}(0,\sigma_p^2)$ |
| $\sigma_p$ | 運動ノイズの標準偏差 |
| $z_t$ | 時刻 $t$ の観測（最近傍ランドマークまでの距離） |
| $w_t^{(i)}$ | 正規化後の重み |
| $\widetilde{w}_t^{(i)}$ | 正規化前の重み |
| $N_{\text{eff}}$ | 有効パーティクル数（重み集中度の指標） |

### アルゴリズムの詳細

#### 1. 予測ステップ（時刻 $t-1$ → $t$）

各パーティクル $i$ の位置を制御入力 $u_t$ に基づいて更新します：

$$x_t^{(i)} = x_{t-1}^{(i)} + u_t + w_t$$

ここで $w_t \sim \mathcal{N}(0, \sigma_p^2)$ はロボットの移動ノイズです。

**意味**: ロボットの移動命令は完全ではなく、実際の動きには不確実性（ノイズ）が伴うため、各パーティクルに異なる移動を適用します。

#### 2. 尤度計算（尤度関数）

時刻 $t$ でセンサ観測 $z_t$ を得たとき、パーティクル $i$ の尤度は：

$$p(z_t \mid x_t^{(i)}) = \frac{1}{\sqrt{2\pi \sigma_m^2}} \exp\left(-\frac{(z_t - d(x_t^{(i)}))^2}{2\sigma_m^2}\right)$$

ここで：
- $d(x_t^{(i)})$: パーティクル $i$ の位置から最近傍ランドマークまでの距離（予測観測値）
- $z_t$: 実際のセンサ観測値
- $\sigma_m$: 観測ノイズの標準偏差

**意味**: 観測値に近い距離を予測するパーティクルほど、尤度が高い（信頼度が高い）と評価されます。

#### 3. 重み更新（ベイズフィルタ）

未正規化重み（前ステップの重みと尤度の積）:

$$\widetilde{w}_t^{(i)} = w_{t-1}^{(i)} \cdot p(z_t \mid x_t^{(i)})$$

正規化（確率分布化）:

$$w_t^{(i)} = \frac{\widetilde{w}_t^{(i)}}{\sum_{j=1}^{N} \widetilde{w}_t^{(j)}}$$

**意味**: 観測に基づいて各パーティクルの信頼度を更新し、確率として正規化します。

#### 4. リサンプリング（必要時）

有効パーティクル数 $N_{\text{eff}}$ が閾値 $N_{\text{threshold}}$ より小さい場合:

$$N_{\text{eff}} = \frac{1}{\sum_{i=1}^{N} (w_t^{(i)})^2}$$

重みに比例した確率で、$N$ 個のパーティクルを再抽出します：

$$\{x_t^{(i)}\} \sim \text{Multinomial}(\{x_t^{(j)}\}_{j=1}^N, \{w_t^{(j)}\}_{j=1}^N)$$

その後、重みをリセット: $w_t^{(i)} := 1/N$ for all $i$

**意味**: 重みが極端に偏った場合、信頼度の低いパーティクルを削除し、信頼度の高い領域に集中させます。

#### 5. 位置推定

フィルタの最終推定位置は、重み付き平均:

$$\hat{x}_t = \frac{\sum_{i=1}^{N} w_t^{(i)} x_t^{(i)}}{\sum_{i=1}^{N} w_t^{(i)}}$$

推定の信頼度（分散）:

$$\text{Var}(\hat{x}_t) = \sum_{i=1}^{N} w_t^{(i)} (x_t^{(i)} - \hat{x}_t)^2$$

---

## システム構成

```
monte_carlo_localization/
├── particle_filter.py           # メインアルゴリズム実装
└── simulate_and_visualize.py    # シミュレーション & 可視化
```

### ファイル説明

#### `particle_filter.py` (メイン実装)

**主要クラス:**

1. **`Config`** - シミュレーション設定
   - `num_particles`: パーティクル数（デフォルト: 1000）
   - `process_noise`: 移動ノイズの標準偏差
   - `measurement_noise`: センサノイズの標準偏差
   - `landmark_positions`: ランドマークの位置リスト

2. **`ParticleFilter`** - パーティクルフィルタの実装
   - `predict(control)`: 予測ステップ
   - `update(measurement)`: 更新ステップ
   - `resample()`: リサンプリング
   - `filter_step()`: 1ステップ分の処理
   - `estimate_position()`: 現在の位置推定値を取得
   - `get_confidence()`: 推定の信頼度を計算

3. **`Robot`** - 1次元ロボットシミュレータ
   - `move(control)`: ロボットを移動（ノイズ付き）
   - `observe_landmark()`: 最も近いランドマークまでの距離を観測

#### `simulate_and_visualize.py` (実行スクリプト)

**主要関数:**

- `simulate()`: シミュレーションを実行（50ステップ）
- `plot_results()`: 3種類のグラフを生成
  - 位置推定の時系列
  - パーティクル分布の時間変化
  - 推定誤差
- `plot_particle_animation()`: アニメーション生成（GIF）

---

## インストール & 実行

### 前提条件
- Python 3.8以上

### セットアップ

```bash
# リポジトリをクローン
git clone https://github.com/nyantaro723/monte_carlo_localization.git
cd monte_carlo_localization

# 依存パッケージをインストール
pip install -r requirements.txt
```

### 実行方法

```bash
# シミュレーションを実行して可視化を生成
python simulate_and_visualize.py
```

**出力:**
- `simulation_results.png` - 3つのプロット図
- `particle_filter_animation.gif` - パーティクルフィルタのアニメーション
- ターミナルに各ステップの推定結果を表示

---

## 結果例

![Simulation Results](simulation_results.png)

### グラフ 1: 位置推定の精度
- **緑線**: ロボットの真の位置
- **赤点線**: フィルタの推定位置
- **青矢印**: ランドマーク（観測対象）

真の位置に推定位置が追従していることが確認できます。

### グラフ 2: パーティクル分布の時間変化
- **赤・オレンジ・紫・緑の点**: 異なる時刻でのパーティクル位置
- 時間とともにパーティクルが真の位置周辺に集中していく様子が見られます

### グラフ 3: 推定誤差
- 初期段階では誤差が大きい
- 観測を繰り返すことで誤差が減少（収束）

### アニメーション: パーティクルフィルタの動作

![Particle Filter Animation](particle_filter_animation.gif)

パーティクルフィルタが時間とともにどのように動作するかを動的に可視化したアニメーション

**各要素の表示:**
- **赤い点**: パーティクル（位置仮説）。点の大きさは各パーティクルの重み（信頼度）に応じて変わります
- **緑の星マーク**: ロボットの真の位置
- **青矢印**: ランドマーク（観測対象）
- **黄色の点**: フィルタの推定位置

**アニメーション中に観察できる現象:**
1. **初期段階（0-10ステップ）**: パーティクルが広く分散していて、様々なサイズ
2. **観測フェーズ（10-30ステップ）**: 尤度が高い領域のパーティクル（真の位置に近い）のサイズが大きくなり、集中し始める
3. **収束フェーズ（30-50ステップ）**: パーティクルが真の位置周辺に集束し、推定位置（黄色）が真の位置（緑）に接近
3. **収束フェーズ（30-50ステップ）**: パーティクル群が真の位置に集束し、推定が安定化

---

## カスタマイズ例

### 1. パラメータの調整

```python
from particle_filter import ParticleFilter, Robot, Config

# カスタム設定
config = Config(
    world_size=200.0,           # ワールドサイズを拡大
    num_particles=5000,         # パーティクル数を増加
    process_noise=2.0,          # ロボットの移動ノイズを増加
    measurement_noise=2.0,      # センサの精度を向上
    landmark_positions=[10, 50, 100, 150, 190]  # ランドマーク数を増加
)

robot = Robot(50.0, config)
pf = ParticleFilter(config)
```

### 2. 異なる移動パターン

```python
# 直線運動でなく、周期的な運動をシミュレート
for step in range(100):
    control = 5.0 + 3.0 * np.sin(2 * np.pi * step / 20)
    robot.move(control)
    measurement = robot.observe_landmark()
    pf.filter_step(control, measurement)
```

### 3. ノイズ特性の実験

```python
# ノイズなし（理想的）
config_ideal = Config(process_noise=0.1, measurement_noise=0.1)

# 高ノイズ（現実的）
config_realistic = Config(process_noise=1.5, measurement_noise=5.0)
```

---

## パラメータ選択ガイド

### パーティクル数 (`num_particles`)

| パーティクル数 | 特性 | 推奨用途 |
|-------|------|--------|
| 100-500 | 高速・低精度 | 初期実験、デバッグ |
| 1000-5000 | 標準 | 本格シミュレーション |
| 5000+ | 低速・高精度 | 精密計測が必要な場合 |

### ノイズ標準偏差

- **`process_noise` が大きい** → ロボットの移動が不確実
- **`measurement_noise` が大きい** → センサが不正確

実際のロボットのスペックに合わせて調整してください。

### リサンプリング閾値 (`resample_threshold`)

- `0.1` - 積極的にリサンプリング（計算量増加）
- `0.5` - 標準（推奨）
- `0.9` - 控えめにリサンプリング（多様性保持）

---

## 動作確認済み環境

- Ubuntu 22.04, 24.04
- macOS 14 (Sonoma)
- Python 3.8, 3.9, 3.10, 3.11, 3.12

---

## 参考文献

- "Particle filter" (Wikipedia), https://en.wikipedia.org/wiki/Particle_filter
- Ryuichi Ueda, 「確率ロボティクス 2025 授業資料 Lesson 6-2」, https://ryuichiueda.github.io/slides_marp/prob_robotics_2025/lesson6-2
- Ryuichi Ueda, 「確率ロボティクス 2025 授業資料 Lesson 7-2」, https://ryuichiueda.github.io/slides_marp/prob_robotics_2025/lesson7-2

---

## ライセンス

- このプロジェクトはMITライセンスの下で公開されています。  
- 詳細は [LICENSE](LICENSE) ファイルを参照してください。

© 2025 Ryusei Matsuki